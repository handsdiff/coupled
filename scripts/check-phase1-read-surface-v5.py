#!/usr/bin/env python3
"""Checks for full-pane authority plus a comparison-only interior projection."""

from __future__ import annotations

from phase1_read_surface_v3 import proposal_for_record as v3_proposal
from phase1_read_surface_v5 import proposal_for_record as v5_proposal
...
def main() -> int:
    ...
    after = v5_proposal(record)["proposal"]

    assert after["selectedDepth"] == before["selectedDepth"] == 1
    assert after["method"] == before["method"] == "ax_semantic_container"
    assert after["screenRectangle"] == before["screenRectangle"]
    assert after["normalizedTopRectangle"] == before["normalizedTopRectangle"]
    assert after["regionOfInterest"] == before["regionOfInterest"]
    assert after["comparisonScreenRectangle"] == {
        "x": 200.0, "y": 260.0, "width": 600.0, "height": 480.0,
    }
    assert after["comparisonNormalizedTopRectangle"] == {
        "x": 0.2, "y": 0.26, "width": 0.6, "height": 0.48,
    }
    assert after["comparisonRegionOfInterest"] == {
        "x": 0.2, "y": 0.26, "width": 0.6, "height": 0.48,
    }
    ...
    assert after["ruleVersion"] == "ax-pane-read-v5"
    print("Phase 1 read-surface-v5 comparison projection checks passed.")
    return 0
