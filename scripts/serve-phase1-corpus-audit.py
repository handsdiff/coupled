    ...
    try:
        with path.open(encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, 1):
                ...
                if not isinstance(value, dict):
                    raise AuditError(f"expected object at {path}:{line_number}")
                rows.append({key: value.get(key) for key in retained_keys})
    except (OSError, json.JSONDecodeError) as error:
        raise AuditError(f"cannot read {path}: {error}") from error
    return rows
...
class AuditStore:
    ...
    def _load_read_provenance(
        ...
        causal_candidates = list(
            (self.project / "coupled-data").glob("*-causal-v15*-final/dataset.json")
        )
    ...
    def _sample_examples(self, sample_size: int) -> list[dict[str, Any]]:
        if sample_size <= 0:
            raise AuditError("sample size must be positive")
    ...
    def detail(self, example_id: str) -> dict[str, Any]:
        ...
        retained = []
        for ordinal, item in enumerate(plan.get("retainedContextBlocks", [])):
        ...
        return {
            ...
            "contextPlan": {
                ...
                "inputTokenBudget": plan.get("inputTokenBudget"),
                "modelInputTokenCount": plan.get("modelInputTokenCount"),
                "droppedContextEventCount": plan.get("droppedContextEventCount"),
            },
        }


HTML = r'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Coupled · corpus audit</title>
<style>
:root{color-scheme:dark;--bg:#090b0e;--panel:#11151a;--panel2:#171c22;--line:#2a333d;--text:#edf2f7;--muted:#96a3b2;--ax:#4ade80;--fallback:#fbbf24;--legacy:#94a3b8;--write:#60a5fa}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:13px/1.45 ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.shell{display:grid;grid-template-columns:360px minmax(0,1fr);min-height:100vh}.side{height:100vh;position:sticky;top:0;overflow:auto;border-right:1px solid var(--line);padding:16px}.main{padding:20px;min-width:0}.muted{color:var(--muted)}h1{font-size:18px;margin:0 0 4px}.stats{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin:14px 0}.stat{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:9px}.stat b{font-size:19px;display:block}.filters{display:flex;gap:7px;margin-bottom:10px}select,input,button{font:inherit;color:var(--text);background:var(--panel2);border:1px solid var(--line);border-radius:7px;padding:7px;min-width:0}.filters input{flex:1}.item{display:block;width:100%;text-align:left;background:transparent;margin:3px 0;padding:9px}.item.active{background:var(--panel2);border-color:#526170}.item small{display:block;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.top{display:flex;gap:7px;align-items:center;flex-wrap:wrap}.top h2{margin:0 auto 0 0;font-size:18px}.badge{display:inline-block;border:1px solid var(--line);border-radius:99px;padding:2px 7px;font:11px ui-monospace,SFMono-Regular,Menlo,monospace}.ax_tree{color:var(--ax);border-color:color-mix(in srgb,var(--ax) 55%,var(--line))}.ax_fallback{color:var(--fallback);border-color:color-mix(in srgb,var(--fallback) 55%,var(--line))}.legacy,.unresolved{color:var(--legacy)}.write{color:var(--write)}.panel{border:1px solid var(--line);border-radius:9px;background:var(--panel);margin-top:12px;overflow:hidden}.panelhead{display:flex;align-items:center;gap:7px;padding:10px 12px;border-bottom:1px solid var(--line)}.panelbody{padding:12px}.panel h3{margin:0;font-size:13px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.grid .panel{min-width:0}.event summary{display:flex;align-items:center;gap:7px;padding:9px 11px;cursor:pointer}.event .panelbody{border-top:1px solid var(--line)}pre{margin:0;white-space:pre-wrap;overflow-wrap:anywhere;max-height:480px;overflow:auto;font:12px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace}.target{font:16px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap;overflow-wrap:anywhere}.spacer{flex:1}.legend{display:grid;gap:5px;margin-top:10px}.empty{padding:80px;text-align:center;color:var(--muted)}@media(max-width:900px){.shell{display:block}.side{position:static;height:auto;border-right:0;border-bottom:1px solid var(--line)}.grid{grid-template-columns:1fr}}
</style></head><body><div class="shell"><aside class="side"><h1>Training corpus audit</h1><div id="subtitle" class="muted"></div><div id="stats" class="stats"></div><div class="filters"><select id="app"><option value="">All apps</option></select><input id="search" placeholder="Search target"></div><div id="items"></div><div id="legend" class="legend"></div></aside><main class="main" id="main"><div class="empty">Loading audit sample…</div></main></div>
<script>
const state={meta:null,rows:[],filtered:[],selected:null};const $=id=>document.getElementById(id);const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const pretty=v=>JSON.stringify(v,null,2);
...
function targetSessionBadge(row){const c=row.targetSessionReadCapture==='ax_tree_enabled'?'ax_tree':row.targetSessionReadCapture==='ax_fallback_only'?'ax_fallback':'legacy';return `<span class="badge ${c}">${esc(c==='ax_tree'?'proper AX-pane source session':c==='ax_fallback'?'AX fallback-only source session':'legacy/no-AX source session')}</span>`}
function apply(){const app=$('app').value,q=$('search').value.toLowerCase();state.filtered=state.rows.filter(r=>(!app||r.application===app)&&(!q||(r.target+' '+r.application+' '+r.surfaceKind).toLowerCase().includes(q)));if(!state.filtered.some(r=>r.exampleID===state.selected))state.selected=state.filtered[0]?.exampleID||null;drawList();if(state.selected)select(state.selected);else $('main').innerHTML='<div class="empty">No matching sample rows.</div>'}
function drawList(){$('items').innerHTML=state.filtered.map(r=>`<button class="item ${r.exampleID===state.selected?'active':''}" data-id="${esc(r.exampleID)}"><b>#${r.chronologicalOrdinal+1} · ${esc(r.application)}</b><small>${esc(r.target.slice(0,100)||'∅')}</small><small>${r.microWriteCount} micro-WRITE${r.microWriteCount===1?'':'s'} · ${esc(r.surfaceKind||'unknown surface')}</small></button>`).join('');document.querySelectorAll('.item').forEach(b=>b.onclick=()=>select(b.dataset.id))}
function eventText(p){if(p.kind==='read')return p.content||'';const seg=p.authorshipSegments||[];return seg.length?seg.map(x=>`${x.type==='paste'?'[PASTE] ':'[AUTHORED] '}${x.content||''}`).join('\n'):pretty(p)}
async function select(id){state.selected=id;drawList();$('main').innerHTML='<div class="empty">Loading exact packed context…</div>';const d=await get('/api/example?id='+encodeURIComponent(id));const s=d.summary;const readCounts=Object.entries(s.retainedReadCounts||{}).map(([k,v])=>`<span class="badge ${k}">${v} ${esc(label(k))}</span>`).join('');$('main').innerHTML=`<div class="top"><h2>#${s.chronologicalOrdinal+1} · ${esc(s.application)} · closed substantive WRITE</h2>${targetSessionBadge(s)}<span class="badge write">${s.microWriteCount} micro-WRITE${s.microWriteCount===1?'':'s'}</span>${readCounts}</div><section class="panel"><div class="panelhead"><h3>LOSS-BEARING CLOSED TARGET</h3><span class="spacer"></span><span class="badge write">content + EOS only</span></div><div class="panelbody target">${esc(d.targetText)}</div></section><div class="grid"><section class="panel"><div class="panelhead"><h3>MODEL-FACING DESTINATION</h3></div><div class="panelbody"><pre>${esc(pretty(d.modelFacingDestination))}</pre></div></section><section class="panel"><div class="panelhead"><h3>EPISODE CONSTRUCTION</h3></div><div class="panelbody"><pre>${esc(pretty({targetMetadata:d.targetMetadata,episode:d.episode,targetMask:d.targetMask}))}</pre></div></section></div><section class="panel"><div class="panelhead"><h3>EXACT RETAINED MODEL HISTORY</h3><span class="spacer"></span><span class="muted">Each READ is labeled from immutable semantic evidence.</span></div><div class="panelbody" id="events"></div></section><section class="panel"><div class="panelhead"><h3>CONDITIONING QUERY</h3></div><div class="panelbody"><pre>${esc(pretty(d.conditioningQuery))}</pre></div></section>`;$('events').innerHTML=d.retainedEvents.map((e,i)=>{const p=e.projection,prov=e.readProvenance;const category=prov?.category||'write';const badge=p.kind==='read'?`<span class="badge ${category}" title="${esc(prov.description)}">${esc(prov.label)}</span>`:'<span class="badge write">closed WRITE history</span>';const audit=prov?`<pre>${esc(pretty({ruleVersion:prov.ruleVersion,captureScope:prov.captureScope,method:prov.method,reason:prov.reason,confidence:prov.confidence,selectedDepth:prov.selectedDepth,selectedRole:prov.selectedRole,selectedSubrole:prov.selectedSubrole}))}</pre>`:'';return `<details class="panel event" ${i>=d.retainedEvents.length-4?'open':''}><summary><span class="muted">${i+1}/${d.retainedEvents.length}</span><span class="badge ${p.kind==='read'?category:'write'}">${esc((p.kind||'event').toUpperCase())}</span><b>${esc(p.application||p.destination?.application||'Unknown app')}</b>${badge}${e.contentTruncated?'<span class="badge ax_fallback">oldest event truncated</span>':''}<span class="spacer"></span><span class="muted">${esc(e.availableAt||'')}</span></summary><div class="panelbody"><pre>${esc(eventText(p))}</pre>${audit}</div></details>`}).join('')}
async function init(){[state.meta,state.rows]=await Promise.all([get('/api/meta'),get('/api/examples')]);$('subtitle').textContent=`${state.meta.sample.method} · seed ${state.meta.sample.seed} · ${state.meta.episodeVersion}`;const c=state.meta.counts,reads=c.corpusReadEvents;$('stats').innerHTML=`<div class="stat"><b>${c.lossBearingClosedWrites}</b>closed targets</div><div class="stat"><b>${state.meta.sample.examples}</b>sampled targets</div><div class="stat"><b>${reads.ax_tree||0}</b>proper AX-pane READs</div><div class="stat"><b>${reads.ax_fallback||0}</b>AX fallback READs</div><div class="stat"><b>${reads.legacy||0}</b>legacy/no-AX READs</div>`;$('legend').innerHTML=Object.entries(state.meta.readProvenanceLegend).filter(([k])=>k!=='unresolved').map(([k,v])=>`<div><span class="badge ${k}">${esc(v.label)}</span> <span class="muted">${esc(v.description)}</span></div>`).join('');[...new Set(state.rows.map(r=>r.application))].sort().forEach(v=>$('app').insertAdjacentHTML('beforeend',`<option>${esc(v)}</option>`));$('app').onchange=$('search').oninput=apply;apply()}
init().catch(e=>$('main').innerHTML=`<pre>${esc(e.stack||e)}</pre>`)
</script></body></html>'''
...
def main() -> int:
    ...
    print(
        ...
        f"showing {len(store.sample)} deterministic sampled cases.",
        flush=True,
    )
